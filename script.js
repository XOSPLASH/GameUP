...
function addTask() {
    ...
    if (taskText) {
        ...
        taskItem.className = difficulty; // Apply the difficulty class
        taskItem.innerHTML = `
            ${taskText}
...
function loadTasks() {
    ...
    tasks.forEach(task => {
        const taskItem = document.createElement('li');
        taskItem.className = task.difficulty;
        taskItem.innerHTML = `
            ${task.text}
...
function loadInventory() {
    ...
    inventory.forEach(item => {
        ...
        itemElement.innerHTML = `
            <img src="${item.icon}" alt="${item.name}" style="width: 24px; height: 24px; margin-right: 10px;">
            ${item.name}
        `;
